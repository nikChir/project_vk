"""
Сравнение "до/после": прогоняет ту же выборку из 8 примеров val.json
(тот же RANDOM_SEED=42, тот же источник данных), что использовалась
в baseline.py, но теперь через модель с применённым LoRA-адаптером.

Перед генерацией явно проверяется, что отобранные id совпадают с id
из уже сохранённого outputs/logs/baseline_results.json — гарантия,
что сравнение "до/после" ведётся на одних и тех же примерах.

Результат сохраняется в outputs/logs/after_training_results.json.
"""
